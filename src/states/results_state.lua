...
local SaveSystem = require("src.systems.save_system")  -- VETS-59
local Scoring = require("src.systems.scoring")  -- VETS-59
...
function ResultsState:enter(data)
    ...
    self.input:init()
    -- Update immediately to consume any held buttons from previous state (VETS-58 fix)
    self.input:update()

    -- Calculate rank using Scoring system (VETS-59)
    local rank = Scoring.getRank(self.score)
    local rank_color = Scoring.getRankColor(rank)
    self.rank = rank
    self.rank_color = rank_color

    -- Update save data if successful (VETS-59)
...
-- Update save data after completing a night (VETS-59)
-- Marks night as completed, updates best score/rank, unlocks next night
function ResultsState:updateSaveData()
    print("[ResultsState] Updating save data for Night " .. self.night_number)

    -- Update progress: completion, best score, best rank, and unlock next night
    local success = SaveSystem.updateNightProgress(self.night_number, self.score, self.rank)

    if success then
        print("[ResultsState] Save data updated successfully")
    else
        print("[ResultsState] Warning: Failed to update save data")
    end
end
...
function ResultsState:drawUI()
    ...
    -- Rank display (VETS-59)
    y_offset = y_offset + 60
    love.graphics.setColor(self.rank_color[1], self.rank_color[2], self.rank_color[3])
    local rank_text = "Rank: " .. self.rank
...
-- Switches to appropriate state based on selection (VETS-59)
function ResultsState:confirmSelection()
    ...
    if self.selected_option == 1 then
        -- Continue - return to night select (VETS-59)
        -- Next night will be unlocked by updateSaveData() if successful
        print("[ResultsState] Option 1 selected: Return to Night Select")
        self.state_manager:switch("night_select", "", self.state_manager)
    elseif self.selected_option == 2 then
        -- Restart - replay current night
